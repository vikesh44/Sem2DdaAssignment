...
export class PersonAddressService {
  ...
  getAllPersonAddress(personId: string | null): any {
    return this.http.get<any>(
      environment.baseUrl + ApiPaths.PersonAddress + '/' + personId
    );
  }

  addPersonAddress(newPersonAddress: PersonAddress): any {
    return this.http.post(
      environment.baseUrl + ApiPaths.PersonAddress,
      JSON.stringify(newPersonAddress),
  ...
  updatePersonAddress(personAddress: PersonAddress): any {
    return this.http.put(
      environment.baseUrl +
        ApiPaths.PersonAddress +
        '/' +
        personAddress.personId,
      JSON.stringify(personAddress),
      { headers: this.headers }
    );
  }
